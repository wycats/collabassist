...
- **Structure**: A horizontal rail of nodes. Each node represents an accepted decision (e.g., "Workspace Model v2"). _(Note: This visualization is a hypothesis; we will iterate based on usage.)_
...
- **No Dead Code**: We remove the old "happy path" hacks _as_ we replace them, not after.
...
### Slice 5: Stabilized Gateway Card Loop

**Goal**: Restore one trustworthy loop before new product work:
prompt -> interpret -> commit intent -> propose -> commit path -> inspect -> accept/pin artifact.

### Implemented

- `src/lib/server/ai/card-generator.ts` provides deterministic `fake` mode and Vercel AI Gateway `gateway` mode.
- `src/lib/ai/schemas.ts`, `src/lib/cards/types.ts`, and `src/lib/schema/card.schema.json` now agree on card shape.
- `/api/cards` validates requests and returns `ErrorCard` data for invalid card requests.
- `/api/decisions` validates accepted cards and returns ISO timestamp decision nodes.
- `+page.svelte` persists real `parentId` values from the selected rail head.
- Lens/mockup inspect cards now need explicit Accept before they become the pinned artifact.
- Fork/refine candidates remember the accepted source decision's parent so accepting them creates a real alternate branch path.
- AI SDK integration is pinned to AI SDK 6 stable for this slice. AI SDK 7 beta is deferred behind the card-generator adapter.
- `docs/ai-gateway-setup.md` documents Vercel CLI + Gateway setup.
- Playwright Chromium headless shell is installed in the local cache after the default Playwright downloader timed out; unit browser tests and E2E can now run.

### Remaining

- Verify live Gateway mode after Vercel login/link/env pull.
- Keep the Node 26 + pnpm environment verified as dependencies move. The repo now declares Node 26 directly, so the earlier runtime-shim follow-up is no longer needed.
- Refine the version model for accepted refinements and forks beyond the current candidate-branch parent behavior.

### Slice 6: Project Alpha Resumable Product Spec Workspace

**Goal**: Turn the stabilized loop into a single-user beta workspace for product-spec work.

### Implemented

- `projects` and `artifacts` are first-class SQLite entities, and old local decisions migrate into a default project.
- `/` loads one active project, filtered decisions, project artifacts, and branch-aware active artifact state.
- The sidebar includes a compact project selector and new-project action.
- `/api/decisions` now commits accepted cards for a project and atomically creates a `product-spec` artifact when a mockup or lens card is accepted.
- The canvas renders the persisted product-spec artifact brief plus its attached mockup/lens payload.
- Fake and Gateway card generation receive project id, seed prompt, active decisions, and current artifact summary.
- Unit coverage verifies the product-spec artifact builder; E2E verifies prompt -> interpret -> propose -> inspect -> accept -> reload persistence and second-project isolation.

### Remaining

- Verify live Gateway mode after Vercel login/link/env pull.
- Keep the Node 26 + pnpm environment verified as dependencies move. The repo now declares Node 26 directly, so the earlier runtime-shim follow-up is no longer needed.
- Refine the version model for accepted refinements and forks beyond the current branch-aware artifact behavior.
